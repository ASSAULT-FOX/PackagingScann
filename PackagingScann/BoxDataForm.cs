...
{
    public partial class BoxDataForm : Form
    {
        String SignWork;
        public BoxDataForm(String SignWork)
        {
            InitializeComponent();
            this.SignWork = SignWork;
            if(SignWork != "HUAWEI")
            {
                this.dataGridView1.DataSource = DalHelper.QueryHUAWEIBox("");
                int width = 0;
            ...
            {
                this.dataGridView1.DataSource = DalHelper.QueryHONORBox("");
                int width = 0;
                for (int i = 0; i < this.dataGridView1.Columns.Count; i++)
                {
                    //将每一列都调整为自动适应模式
        ...
        {   
            if(SignWork == "HUAWEI")
            {
                this.dataGridView1.DataSource = DalHelper.QueryHUAWEIBox(this.tiaomainfo.Text);
            }
            else
            {
                this.dataGridView1.DataSource = DalHelper.QueryHONORBox(this.tiaomainfo.Text);
            }
            ...
            if(SignWork == "HUAWEI")
            {
                ...
                if (!string.IsNullOrEmpty(PM))
                {
                    ...
                    if (PM == "jajqr168" || passwordtext == Password)
                    {
                        ...
                        if (DalHelper.DeleteHUAWEIBox((string)this.dataGridView1.SelectedRows[0].Cells[0].Value))
                        {
                            MessageBox.Show("手动删除外箱数据成功");
                            this.dataGridView1.DataSource = DalHelper.QueryHUAWEIBox(this.tiaomainfo.Text);
                            dataGridView1.Refresh();
                        }
                    }
            ...
            else
            {
                ...
                if (!string.IsNullOrEmpty(PM))
                {
                    ...
                    if (PM == "jajqr168" || passwordtext == Password)
                    {
                        ...
                        if (DalHelper.DeleteHONORBox((string)this.dataGridView1.SelectedRows[0].Cells[0].Value))
                        {
                            MessageBox.Show("手动删除外箱数据成功");
                            this.dataGridView1.DataSource = DalHelper.QueryHONORBox(this.tiaomainfo.Text);
                            dataGridView1.Refresh();
