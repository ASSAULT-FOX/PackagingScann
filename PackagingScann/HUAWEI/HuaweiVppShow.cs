...
{
    public partial class HuaweiVppShow : UIForm
    {
        public static HuaweiVppShow thisForm;

        public HuaweiVppShow()
        {
            InitializeComponent();
        }
        ...
        public static HuaweiVppShow GetInstance(bool isActive = true)
        {
            //是否自动创建 不自动创建时
            if (thisForm == null || thisForm.IsDisposed)
            {
                //窗口首次打开 或者 被关闭后再次打开
                thisForm = new HuaweiVppShow();
            }
            else
            {
                if (isActive)
        ...
        {
            string VPPName = $"{ParameterSetClass.QueryHuaweiParameterInfo("物料编码", "视觉")}.vpp";
            debugVppPath = Path.Combine(Application.StartupPath, "Vpp", VPPName);
            toolBlockEdit.Subject = HUAWEIForm.ScanCode1_Block;
        }
