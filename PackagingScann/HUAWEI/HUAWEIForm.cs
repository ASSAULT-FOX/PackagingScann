...
{
    public partial class HUAWEIForm : Form
    {
        ...
        public DateTime BoxDateHuawei; //华为工单日期  用于请求产品条码格式 20240704
        ...
        public HUAWEIForm()
        {
            ...
            this.Width = 1103;
            InitializeComponent();
            ParameterSetClass.HuaweiTaskTimeCheck();
        }
        ...
        private void HonorForm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                //加载VPP文件
                string VPPName = ParameterSetClass.QueryHuaweiParameterInfo("物料编码", "视觉");
        ...
        private void FrmLoad(object sender, DoWorkEventArgs e)
        {
            try
            {
                string xiaohao = ParameterSetClass.QueryHuaweiParameterInfo("箱号", "MES");
                string BoxID = ParameterSetClass.QueryHuaweiParameterInfo("箱码", "MES");
                string ASN = ParameterSetClass.QueryHuaweiParameterInfo("ASN", "MES");
                string UPNum = ParameterSetClass.QueryHuaweiParameterInfo("当前箱已扫描数量", "MES");

                string workorder = ParameterSetClass.QueryHuaweiParameterInfo("工单号", "MES");
                string ItemNo = ParameterSetClass.QueryHuaweiParameterInfo("料号", "MES");
                string site = ParameterSetClass.QueryHuaweiParameterInfo("站点", "MES");
                string LineBody = ParameterSetClass.QueryHuaweiParameterInfo("线体", "MES");
                string ComputerHost = ParameterSetClass.QueryHuaweiParameterInfo("电脑主机", "MES");
                string UserID = ParameterSetClass.QueryHuaweiParameterInfo("工号", "MES");
                string PackagingType = ParameterSetClass.QueryHuaweiParameterInfo("是否箱包装", "MES");
                string CPIsHeavyIndustry = ParameterSetClass.QueryHuaweiParameterInfo("产品是否重工", "MES");
                string WXIsHeavyIndustry = ParameterSetClass.QueryHuaweiParameterInfo("外箱是否重工", "MES");
                if (!string.IsNullOrEmpty(workorder) && !string.IsNullOrEmpty(ItemNo) && !string.IsNullOrEmpty(site) && !string.IsNullOrEmpty(LineBody) && !string.IsNullOrEmpty(ComputerHost) && !string.IsNullOrEmpty(UserID) && !string.IsNullOrEmpty(PackagingType) && !string.IsNullOrEmpty(CPIsHeavyIndustry) && !string.IsNullOrEmpty(WXIsHeavyIndustry))
                {
                    sttryy();
                    ...
                    {
                        WarnForm boxWarnForm = new WarnForm($"{xiaohao}\r\n这箱只扫描 {UPNum} PCS，还没扫描完\r\n\r\n继续扫描：继续扫描这一箱\r\n扫描新箱：扫描新的箱子", "继续扫描", "扫描新箱", WarnFormPasswordEnabled, WarnFormPasswordEnc);
                        if (boxWarnForm.ShowDialog() == DialogResult.OK)
                        {
                            this.Invoke((Action)(() =>
                            {
                                strNumber();
                                this.textBox3.Text = ASN;
                                this.textBox3.Enabled = false;
                                this.textBox1.Text = BoxID;
                                this.textBox1.Enabled = false;
                                this.label21.Text = xiaohao;
                                this.label19.Text = $"{UPNum} PCS";
                                this.label3.Text = "已恢复之前的数据，可以开始扫描产品！";
                                this.label3.ForeColor = Color.Green;
                                this.Box_HWPSN.Enabled = false;
                                this.textBox2.Focus();
                            }));
                            LogHelper.WriteLog($"【{DateTime.Now:HH:mm:ss:ffff}】恢复{xiaohao}的数据，继续开始扫描" + Environment.NewLine);
                        }
                        else
                        {
                            //清空箱号，ASN，已扫描数量
                            ParameterSetClass.UpdateHuaweiData("箱号", null, "MES");
                            ParameterSetClass.UpdateHuaweiData("箱码", null, "MES");
                            ParameterSetClass.UpdateHuaweiData("ASN", null, "MES");
                            ParameterSetClass.UpdateHuaweiData("当前箱已扫描数量", "0", "MES");
                            this.Invoke((Action)(() =>
                            {
                                this.textBox3.Focus();
                            }));
                            LogHelper.WriteLog($"【{DateTime.Now:HH:mm:ss:ffff}】放弃{xiaohao}的数据，开始扫描新箱" + Environment.NewLine);
                        }
                    }
                    else
                    {
        ...
        {
            string LastVPPName = ParameterSetClass.QueryHuaweiParameterInfo("物料编码", "视觉");
            ...
            if (comboBox1.SelectedItem != null)
            {
                try
                {
                    ...
                    // 数据库保存
                    ParameterSetClass.UpdateHuaweiData("物料编码", comboBox1.SelectedItem.ToString(), "视觉");
                }
                catch (Exception ex)
                {
                    DalHelper.WriteLogInfo(ex.Message);
        ...
        private void HandleScannerStatusUpdated()
        {
            switch (Work_Run_Case)
            {
                ...
                case 2:
                    Work_Run_Case = 0;
                    //获取条码样式
                    BarCodeStyle = ParameterSetClass.QueryHuaweiParameterInfo("条码样式", "MES");
                    this.Invoke((Action)(() =>
                    {
                        this.label3.Text = "";
        ...
        private void HonorForm_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Down)
            {
                ...
                {
                    MessageBox.Show("箱码没有全部扫描！需要先扫描完外箱的两个条码！");
                }
            } 
        }
    ...
    //            {
				//	HuaweiCanShuForm csf = new HuaweiCanShuForm();
				//	csf.ShowDialog(this);
				//}
    //            else
    //            {
    ...
    //        }
        ...
        private void UploadMES()
        {
            try
            {
                ...
                Parallel.Invoke(
                    () => workorder = ParameterSetClass.QueryHuaweiParameterInfo("工单号", "MES"),
                    () => ItemNo = ParameterSetClass.QueryHuaweiParameterInfo("料号", "MES"),
                    () => site = ParameterSetClass.QueryHuaweiParameterInfo("站点", "MES"),
                    () => sitexuhao = ParameterSetClass.QueryHuaweiParameterInfo("站点序号", "MES"),
                    () => Scanxuhao = ParameterSetClass.QueryHuaweiParameterInfo("扫描序号", "MES"),
                    () => LineBody = ParameterSetClass.QueryHuaweiParameterInfo("线体", "MES"),
                    () => ComputerHost = ParameterSetClass.QueryHuaweiParameterInfo("电脑主机", "MES"),
                    () => UserID = ParameterSetClass.QueryHuaweiParameterInfo("工号", "MES"),
                    () => PackagingType = ParameterSetClass.QueryHuaweiParameterInfo("是否箱包装", "MES"),
                    () => CaseNumber = ParameterSetClass.QueryHuaweiParameterInfo("箱号", "MES"),
                    () => IsHeavyIndustry = ParameterSetClass.QueryHuaweiParameterInfo("产品是否重工", "MES"),
                    () => WXIsRepaired = ParameterSetClass.QueryHuaweiParameterInfo("产品是否维修品", "MES")
                );
                ...
                string HwSN = textBox3.Text.Trim();
                ...
                foreach (var code in CodeByID)
                {
                    ...
                    //保存原始数据
                    Task.Run(() => DalHelper.WriteData(code.Value, CaseNumber, BoxSN, HwSN, str));
                }
                ...
                //保存已上传数量
                ParameterSetClass.UpdateHuaweiData("当前箱已扫描数量", UPNum, "MES");
                ...
                if (FullBox)
                {
                    if (HWPSNCheck.Checked)
                    {
                        this.Invoke((Action)(() =>
                        {
                            this.Box_HWPSN.Enabled = true;
                            this.Box_HWPSN.Text = "";
                            this.Box_HWPSN.Focus();
                            using (WaitForScanner waitForm = new WaitForScanner("该箱已满,请扫描华为PSN"))
                            {
                                if (waitForm.ShowDialog() == DialogResult.OK)
                                {
                                    string scannerText = waitForm.ScannerText;
                                    // 在这里可以使用返回的数据做其他操作
                                    Box_HWPSN.Text = scannerText;
                                    KeyEventArgs keyEventArgs = new KeyEventArgs(Keys.Enter);
                                    Box_HWPSN_KeyDown(Box_HWPSN, keyEventArgs);
                                }
                            }
                        }));
                    }
        ...
        {
            string workorder = ParameterSetClass.QueryHuaweiParameterInfo("工单号", "MES");
            string boxDate= ParameterSetClass.QueryHuaweiParameterInfo("工单日期", "MES");  
            try
            {
                if (!string.IsNullOrEmpty(workorder))
                {
        ...
        public void sttryy()
        {
			this.Invoke((Action)(() => 
            {
				this.label15.Text = ParameterSetClass.QueryHuaweiParameterInfo("工单号", "MES");
				this.label16.Text = ParameterSetClass.QueryHuaweiParameterInfo("当前工单总数量", "MES") + " PCS";
				//this.label17.Text = ParameterSetClass.QueryHuaweiParameterInfo("当前工单扫描箱数", "MES") + " 箱";
				this.label18.Text = ParameterSetClass.QueryHuaweiParameterInfo("当前工单每箱数量", "MES") + " PCS";
				//this.label21.Text = ParameterSetClass.QueryHuaweiParameterInfo("箱号", "MES");
			}));
        }
        #endregion
        ...
        #region 华为PSN框回车事件
        private void Box_HWPSN_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyValue != 13) return;

            string workorder = ParameterSetClass.QueryHuaweiParameterInfo("工单号", "MES");

            string HWPSN = this.Box_HWPSN.Text.Trim();

            if (!string.IsNullOrEmpty(workorder) && !string.IsNullOrEmpty(HWPSN))
            {
                string str = ws.GetCheckHWPSN(workorder, HWPSN);

                LogHelper.WriteLog($"华为PSN校验:MES返回结果:{str}");
                ...
                if (str.StartsWith("OK"))
                {
                    ...
                    this.chengxujilu.AppendText($"【{DateTime.Now:HH:mm:ss:ffff}】华为PSN扫描校验成功：{str}" + Environment.NewLine);
                    this.textBox1.Enabled = true;
                    this.textBox3.Enabled = true;
                    this.textBox1.Text = "";
                    this.textBox3.Text = "";
                    this.textBox3.Focus();
                    this.label3.Text = "华为PSN扫描校验成功！！！";
                    this.label3.ForeColor = Color.Green;
                    this.label19.Text = "";
                    this.label21.Text = "";
                    this.Box_HWPSN.Text = "";
                ...
                {
                    this.chengxujilu.AppendText($"【{DateTime.Now:HH:mm:ss:ffff}】华为PSN扫描校验失败：{str}" + Environment.NewLine);
                    this.Box_HWPSN.Focus();
                    this.Box_HWPSN.Text = "";
            ...
            else
            {
                this.Box_HWPSN.Focus();
                this.Box_HWPSN.Text = "";
                using (WaitForScanner waitForm = new WaitForScanner("PSN不能为空，需要扫描PSN"))
                {
                    if (waitForm.ShowDialog() == DialogResult.OK)
                    {
                        string scannerText = waitForm.ScannerText;
        ...
        private void textBox3_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyValue == 13)
            {
                ...
                string ItemNo = ParameterSetClass.QueryHuaweiParameterInfo("料号", "MES");
                string site = ParameterSetClass.QueryHuaweiParameterInfo("站点", "MES");
                string IsHeavyIndustry = ParameterSetClass.QueryHuaweiParameterInfo("外箱是否重工", "MES");
                ...
                {
                    string HWBarcode = this.textBox3.Text.Trim();

                    if (!string.IsNullOrEmpty(HWBarcode))
                    {
                        if (DalHelper.CheckHuaweiASN(HWBarcode))
                        {
                            this.textBox1.Focus();
                            textBox3.Enabled = false;

                            DalHelper.InsertHuaweiASN(HWBarcode);

                            this.label3.Text = "华为ASN扫描成功！！！";
                            this.label3.ForeColor = Color.Green;
                            this.chengxujilu.AppendText($"【{DateTime.Now:HH:mm:ss:ffff}】已扫描华为ASN：{HWBarcode}" + Environment.NewLine);
                        }
                        else
                        {
                            this.chengxujilu.AppendText($"【{DateTime.Now:HH:mm:ss:ffff}】华为条码重复录入，请检查华为条码！！！" + Environment.NewLine);

                            GetWarnFormPassword();
                            WarnForm boxWarnForm = new WarnForm("检测到这个条码之前扫描过", WarnFormPasswordEnabled, WarnFormPasswordEnc);
                            if (boxWarnForm.ShowDialog() == DialogResult.OK)
                            {
                                DalHelper.DeleteHUAWEIASN(HWBarcode);
                                this.textBox1.Focus();
                                textBox3.Enabled = false;
                                this.label3.Text = "华为ASN扫描成功！！！";
                                DalHelper.InsertHuaweiASN(HWBarcode);
                            ...
                            else
                            {
                                ...
                                this.textBox3.Focus();
                                this.label3.Text = "请重新扫码华为ASN！！！";
                                this.label3.ForeColor = Color.Red;
                                this.chengxujilu.AppendText($"【{DateTime.Now:HH:mm:ss:ffff}】请重新扫码华为ASN！！！" + Environment.NewLine);
                            }
                        }
                    }
                    else
                    {
                        this.textBox3.Focus();
                        this.label3.Text = "请扫描华为ASN！！！";
                        this.label3.ForeColor = Color.Red;
                        this.chengxujilu.AppendText($"【{DateTime.Now:HH:mm:ss:ffff}】华为ASN不能为空！！！" + Environment.NewLine);
                    }
                }
                else
                {
        ...
        private void textBox1_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyValue == 13)
            {
                ...
                if (!string.IsNullOrEmpty(workorder) && !string.IsNullOrEmpty(ItemNo) && !string.IsNullOrEmpty(site) && !string.IsNullOrEmpty(IsHeavyIndustry))
                {
                    ...
                    {
                        //webservice接口
                        string MESstr = ws.GetCheckHWASNLXASN(workorder, ItemNo, site, IsHeavyIndustry, "2", this.textBox3.Text.Trim(), OuterBoxBarcode, LineBody, UserID);
                        ...
                        if (MESstr.StartsWith("OK"))
                        {
                            ...
                            if (DalHelper.CheckHuaweiBox(OuterBoxBarcode))
                            {
                                DalHelper.DeleteHUAWEIBox(OuterBoxBarcode);
                                DalHelper.DelBoxSN(xianghao);
                            }

                            //保存箱号和箱码
                            ParameterSetClass.UpdateHuaweiData("箱号", xianghao, "MES"); 
                            ParameterSetClass.UpdateHuaweiData("箱码", OuterBoxBarcode, "MES");
                            ParameterSetClass.UpdateHuaweiData("ASN", this.textBox3.Text.Trim(), "MES");

                            this.label21.Text = xianghao;
                            this.textBox1.Enabled = false;
                            this.textBox3.Enabled = false;
                            this.textBox2.Focus();
                            this.label3.Text = $"OK: {xianghao} 可以开始扫描产品";
                            this.label3.ForeColor = Color.Green;
                            ...
                            //保存原始数据
                            DalHelper.WriteData("", xianghao, OuterBoxBarcode, this.textBox3.Text.Trim(), MESstr);
                            DalHelper.InsertHuaweiBox(OuterBoxBarcode);
                        }
                        else
                        {
                            this.textBox1.Text = "";
                            ...
                            //保存原始数据
                            DalHelper.WriteData("", "", OuterBoxBarcode, this.textBox3.Text.Trim(), MESstr);
                            LogHelper.WriteLog($"【{DateTime.Now:HH:mm:ss:ffff}】{MESstr}" + Environment.NewLine);

                            GetWarnFormPassword();
                            WarnForm boxWarnForm = new WarnForm("箱码未通过MES校验", "重扫箱码", "联系品管", WarnFormPasswordEnabled, WarnFormPasswordEnc);
                            if (boxWarnForm.ShowDialog() == DialogResult.OK)
                            {
        ...
        private void textBox2_Repulse_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyValue == 13)
            {
                ...
                string ItemNo = ParameterSetClass.QueryHuaweiParameterInfo("料号", "MES");
                string site = ParameterSetClass.QueryHuaweiParameterInfo("站点", "MES");
                string sitexuhao = ParameterSetClass.QueryHuaweiParameterInfo("站点序号", "MES");
                string Scanxuhao = ParameterSetClass.QueryHuaweiParameterInfo("扫描序号", "MES");
                string LineBody = ParameterSetClass.QueryHuaweiParameterInfo("线体", "MES");
                string ComputerHost = ParameterSetClass.QueryHuaweiParameterInfo("电脑主机", "MES");
                string UserID = ParameterSetClass.QueryHuaweiParameterInfo("工号", "MES");
                string PackagingType = ParameterSetClass.QueryHuaweiParameterInfo("包装类型", "MES");
                string CaseNumber = ParameterSetClass.QueryHuaweiParameterInfo("箱号", "MES");
                string IsHeavyIndustry = ParameterSetClass.QueryHuaweiParameterInfo("产品是否重工", "MES");
                ...
                {
                    string str = ws.SetSimplePackScan(ProductBarcode, workorder, ItemNo, LineBody, ComputerHost, UserID, site, sitexuhao, Scanxuhao, "1", CaseNumber, "", PackagingType, "N", "N", "N");
                ...
                else
                {
                    ...
                    this.label3.ForeColor = Color.Red;
                    this.chengxujilu.AppendText($"【{DateTime.Now:HH:mm:ss:ffff}】未扫到码或者SN条码样式校验失败，请检查！！！：" + ProductBarcode + Environment.NewLine);
                }
            }
        }
        #endregion
        ...
        {
            BoxDataForm boxDataForm = new BoxDataForm("HUAWEI");
            boxDataForm.Show();
        }

        private void pictureBox5_Click(object sender, EventArgs e)
        {
            ASNDataForm asnDataForm = new ASNDataForm();
            asnDataForm.Show();
        }
        ...
        {
            string sWarnFormPasswordEnabled = ParameterSetClass.QueryHuaweiParameterInfo("允许报警处理密码", "系统");
            ...
            else
            {
                ...
                string wfPasswordEnc = ParameterSetClass.QueryHuaweiParameterInfo("报警处理密码", "系统");
                ...
                {
                    WarnFormPasswordEnc = CryptInfoNameSpace.CryptInfoNameSpace.Encrypt(HuaweiCanShuForm.defaultAPPass);
                }
            }
        }
        #endregion
        ...
        {
            HuaweiVppShow.GetInstance().Show();
        }
        ...
        public  static void WXBMessage(HUAWEIForm form)
        {
            form.label29.Visible = ParameterSetClass.QueryHuaweiParameterInfo("产品是否维修品", "MES") == "Y";
        }
